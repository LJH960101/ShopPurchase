...
namespace ShopPurchase.Core.Thread
{
    ...
    public class JHTimingWheel
    {
        ...
        ///
        /// 여기서 건지는 건 슬롯에 들어와 있는 작업뿐이다. 아직 휠 범위 밖이라 대기 중인 장기
        /// 예약(ArmAfterLongWait)은 그냥 버린다 — 한 시간 뒤에 실행될 일을 종료 시점에 앞당겨
        /// 실행하는 건 예약을 지키는 게 아니라 어기는 것이다. 서버 재시작을 넘어 살아남아야 하는
        /// 예약이라면 애초에 메모리가 아니라 DB에 있어야 한다.
        /// </summary>
        public void Stop()
        {
